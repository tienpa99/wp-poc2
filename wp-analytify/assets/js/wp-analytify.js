	...
	function ($) {
		...
		$("#view_analytics").on('click', function (e) {
			...
			const fetch_stats = async (endpoint, nonce, data = {}) => {
				...
				const URL = `${endpoint}${wp_analytify_script.delimiter}${params.toString()}`;
			...
			$('#pa-single-admin-analytics .show-hide').html('').addClass('stats_loading');
			...
			fetch_stats(wp_analytify_script.url + 'single-post-stats', wp_analytify_script.nonce, fetch_data).then((response) => {
				let markup = '';
					...
					if (response.general_stats) {
						...
						for (const box_key in response.general_stats.stats) {
							...
							markup += `<div class="analytify_general_status_boxes">
									<h4>${box.title}</h4>
									<div class="analytify_general_stats_value">${box.value} ${box.append ? box.append : ``} </div>
					...
					if (response.scroll_depth) {
						markup += `<div class="analytify_general_status analytify_status_box_wraper">
							...
							${response.scroll_depth.stats.length ?
								generate_stats_table(response.scroll_depth.headers, response.scroll_depth.stats, response.scroll_depth.table_class)
								:
							...
							<div class="analytify_status_body">
								<div class="analytify_clearfix">
									<div class="analytify_pull_left analytify_half">
										${response.geographic.sections.countries.stats.length ? generate_stats_table(response.geographic.sections.countries.headers, response.geographic.sections.countries.stats, 'analytify_data_tables analytify_border_th_tp') : empty_stats(wp_analytify_script.no_stats_message)}
									</div>
									<div class="analytify_pull_left analytify_half">
										${response.geographic.sections.countries.stats.length ? generate_stats_table(response.geographic.sections.cities.headers, response.geographic.sections.cities.stats, 'analytify_data_tables analytify_border_th_tp') : empty_stats(wp_analytify_script.no_stats_message)}
									</div>
								</div>
							</div>
					...
					if (response.system_stats) {
						markup += `<div class="analytify_general_status analytify_status_box_wraper">
							...
							<div class="analytify_status_body analytify_clearfix">
								...
								generate_stats_table(response.system_stats.sections.browser.headers, response.system_stats.sections.browser.stats, 'analytify_data_tables')
								:
								empty_stats(wp_analytify_script.no_stats_message, response?.system_stats?.sections?.browser?.headers, 'analytify_data_tables')
							}
								</div>

								<div class="analytify_one_tree_table">
									${response.system_stats.sections.os.stats.length ?
								generate_stats_table(response.system_stats.sections.os.headers, response.system_stats.sections.os.stats, 'analytify_data_tables')
								:
								empty_stats(wp_analytify_script.no_stats_message, response?.system_stats?.sections?.os?.headers, 'analytify_data_tables')
							}
								</div>
								
								<div class="analytify_one_tree_table">
									${response.system_stats.sections.mobile.stats.length ?
								generate_stats_table(response.system_stats.sections.mobile.headers, response.system_stats.sections.mobile.stats, 'analytify_data_tables')
								:
								empty_stats(wp_analytify_script.no_stats_message, response?.system_stats?.sections?.mobile?.headers, 'analytify_data_tables')
							}
								</div>
					...
					if (response.social_media) {
						markup += `<div class="analytify_general_status analytify_status_box_wraper">
							...
							<div class="analytify_status_body">
								...
								generate_stats_table(response.social_media.headers, response.social_media.stats, 'analytify_data_tables analytify_no_header_table')
								:
								empty_stats(wp_analytify_script.no_stats_message)
							}
							</div>
					...
					if (response.referer) {
						markup += `<div class="analytify_general_status analytify_status_box_wraper">
							...
							<div class="analytify_status_body">
								...
								generate_stats_table(response.referer.headers, response.referer.stats, 'analytify_bar_tables')
								:
								empty_stats(wp_analytify_script.no_stats_message)
							}
							</div>
				...
				$('#pa-single-admin-analytics .show-hide').html(markup).removeClass("stats_loading");
