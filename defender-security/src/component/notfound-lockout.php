	...
	public function log_event( $ip, $uri, $scenario ) {
		...
		switch ( $scenario ) {
			case self::SCENARIO_ERROR_404:
				...
				$model->log  = sprintf( __( "Request for file %s which doesn't exist", 'defender-security' ), $uri );
				break;
			...
			default:
				...
				$model->log  = sprintf( __( 'Lockout occurred:  Too many 404 requests for %s', 'defender-security' ), $uri );
				break;
		}
		$model->save();
