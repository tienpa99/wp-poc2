...
class Blocklist_Monitor extends Controller {
	...
	public function export_strings() {
		...
		if ( '1' === (string)$this->get_status() ) {
			$strings = array( __( 'Active', 'defender-security' ) );
		} else {
			$strings = array( __( 'Inactive', 'defender-security' ) );
		}
	...
	public function config_strings( $config, $is_pro ) {
		if ( $is_pro ) {
			$strings = $config['enabled'] ? array( __( 'Active', 'defender-security' ) ) : array( __( 'Inactive', 'defender-security' ) );
		} else {
			$strings = array(
				sprintf(
				/* translators: ... */
