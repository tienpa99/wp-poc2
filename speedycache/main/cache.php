	...
	*/
	...
	static function callback($buffer){
		...
		if(!empty($speedycache->options['combine_js'])){
			...
			$head_new = substr($content, $head_first_index, ($head_last_index - $head_first_index + 1));
		...
		if(file_exists(SPEEDYCACHE_PRO_DIR . '/enhanced.php')){
			...
			if(!empty($speedycache->options['minify_js']) && file_exists(SPEEDYCACHE_PRO_DIR . '/enhanced.php')){
	...
	static function is_cacheable($buffer){
		...
		if(!empty($post_id)){
			...
			if(!empty($post_meta['disable_cache'])){
				return $buffer . '<!-- Speeeify: Cache has been disabled -->';
			}
		}
