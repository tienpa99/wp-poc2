...
if ( empty( $_SESSION['nfw_goodguy'] ) ) {
	$err_msg = sprintf(
		esc_html__('Error: You must be whitelisted in order to use that '.
		'feature: click on the <a href="%s">Firewall Policies</a> menu '.
		'and ensure that the "Add the Administrator to the whitelist" '.
		'option is enabled.', 'ninjafirewall'), '?page=nfsubpolicies');
}
if (! empty( $err_msg ) ) {
	?>
	<div class="wrap">
	<h1><img style="vertical-align:top;" src="<?php
		echo esc_url( plugins_url('/ninjafirewall/images/ninjafirewall_32.png',
			dirname( __FILE__ ) ) )
