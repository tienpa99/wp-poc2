...
function easy_hide_login_hidden_field(){
    $EHL_slug = get_option('wpseh_l01gnhdlwp','');
    ?>
    	<input type="hidden" name="redirect_slug" value="<?php esc_attr_e($EHL_slug) ?>" />
  <?php 
}
...
function easy_hide_login_lostpassword() {
    ...
    $slug =  get_option('wpseh_l01gnhdlwp');
    return site_url("wp-login.php?action=lostpassword&$slug&redirect=false");
}
...
function ehl_easy_hide_login_lostpassword_redirect($lostpassword_redirect) {
	...
	return 'wp-login.php?checkemail=confirm&redirect=false&' . $slug;
}
...
function wp_hide_login_plugin_options() {
   ...
   if (isset($_POST['slug'])) {
    ...
    check_admin_referer('easy-hide-login', '_wpnonce');
 ...
 <input type="text" value="<?php esc_attr_e( $slug ) ?>" name = "slug" class="slug">
 </div>
