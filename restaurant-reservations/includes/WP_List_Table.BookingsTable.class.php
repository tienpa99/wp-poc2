...
class rtbBookingsTable extends WP_List_Table {
	...
	public function get_current_date_range_guest_count() {
		...
		return array_sum( array_column( $this->bookings, 'party' ) );
	}
...
<?php
	...
	public function add_notification() {
		...
		if ( !empty( $this->filter_start_date ) || !empty( $this->filter_end_date ) ) {
			...
			$notifications['count'] = sprintf( _x( 'There are %d bookings with a total of %d guests in this date range.', 'Gives the total number of bookings and guests for a date-filtered range in the admin area.', 'restaurant-reservations' ), sizeof( $this->bookings ), $this->get_current_date_range_guest_count() );
		}
