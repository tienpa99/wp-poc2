...
if ( ! class_exists( 'rtbMultipleLocations', false ) ) {
	...
	class rtbMultipleLocations {
		...
		public function hooks() {
			...
			add_filter( 'rtb_settings_page',                          array( $this, 'maybe_add_location_settings' ), 10, 2 );
		 ...
		 * @since 2.3.6
		 */
		public function maybe_add_location_settings( $sap, $rtb_settings ) {
			global $rtb_controller;
			...
			$sap->add_section(
				...
				array(
					...
					'title'         => __( 'Select Advanced Settings Location', 'restaurant-reservations' ),
			...
			// Section/Table location-specific options
			foreach ( $terms as $term ) {
				...
				$sap->add_setting(
			      ...
			      'infinite_table',
			      array(
			        'id'      => $term->slug . '-rtb-table-sections',
					...
					'fields'    => array_merge(
						array(
			        		...
			        		'section' => array(
			            		...
			            		'required'  => false,
			            		'options'   => $rtb_settings->get_table_section_options( $term->term_id )
			          		),
			        		'combinations' => array(
			            		'type'    => 'text',
			 			...
			 			$rtb_settings->get_table_deposit_column()
			      	)
			      )
			    );
			}
