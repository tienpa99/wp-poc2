...
class BookingAddedEventHandler
{
    ...
    public static function handle($commandResult, $container)
    {
        ...
        if ($commandResult->getData()['packageId']) {
            ...
            $paymentLinksEnabled = $package->getSettings() && json_decode($package->getSettings()->getValue(), true)
                && json_decode($package->getSettings()->getValue(), true)['payments']['paymentLinks'] ?
                json_decode($package->getSettings()->getValue(), true)['payments']['paymentLinks']['enabled'] :
                $settingsDS->getSetting('payments', 'paymentLinks')['enabled'];

            if (!empty($paymentId) && $paymentLinksEnabled && empty($commandResult->getData()['fromLink'])) {
        ...
        foreach ($reservation['bookings'] as $index => $reservationBooking) {
            if ($booking['id'] === $reservationBooking['id']) {
                ...
                $paymentId = $commandResult->getData()['paymentId'];
                if (!empty($paymentId) && !$commandResult->getData()['packageId'] && $paymentLinksEnabled && empty($commandResult->getData()['fromLink'])) {
//                    $data['customer'] = $data['booking']['customer'];
        ...
        foreach ($recurringData as $key => $recurringReservationData) {
            ...
            if (!$commandResult->getData()['packageId'] && $paymentLinksEnabled && empty($commandResult->getData()['fromLink'])) {
                $bookable = $data['bookable'];
                $dataRecurring = $recurringReservationData;
                $dataRecurring['bookable'] = $bookable;
