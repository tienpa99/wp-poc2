...
class AppointmentAddedEventHandler
{
    ...
    public static function handle($commandResult, $container)
    {
        ...
        if (!$pastAppointment) {
            ...
            $paymentLinksEnabled = $appointmentObject->getService()->getSettings() && json_decode($appointmentObject->getService()->getSettings()->getValue(), true)
            && json_decode($appointmentObject->getService()->getSettings()->getValue(), true)['payments']['paymentLinks'] ?
                json_decode($appointmentObject->getService()->getSettings()->getValue(), true)['payments']['paymentLinks']['enabled'] :
            ...
            foreach ($appointment['bookings'] as $index => $booking) {
                if ($booking['status'] === BookingStatus::APPROVED || $booking['status'] === BookingStatus::PENDING) {
                    ...
                    if (!empty($paymentId) && $paymentLinksEnabled) {
