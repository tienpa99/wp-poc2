    ...
    public static function handle($commandResult, $container)
    {
        ...
        if (count($eventsData['edited']) > 0 && !$addedEvents->length() && !$rescheduledEvents->length() && !$deletedEvents->length()) {
            ...
            foreach ($eventsData['edited'] as $event) {
                $emailNotificationService->sendAppointmentUpdatedNotifications($event->toArray());

                if ($settingsService->getSetting('notifications', 'smsSignedIn') === true) {
                    $smsNotificationService->sendAppointmentUpdatedNotifications($event->toArray());
                }

                if ($whatsAppNotificationService->checkRequiredFields()) {
                    $whatsAppNotificationService->sendAppointmentUpdatedNotifications($event->toArray());
                }
            }
        }
