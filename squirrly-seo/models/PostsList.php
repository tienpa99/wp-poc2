        ...
        if (isset($json->posts)) {
            foreach ($json->posts as $post_id => $row) {
                ...
                if (isset($row->optimized) && (int)$row->optimized > 0) {
                    ...
                    $html = '<a class="sq_optimize" href="' . admin_url('post.php?action=edit&post_type=' . $post_type . '&post=' . $post_id) . '" title="'.esc_html__("Optimize it with Squirrly Live Assistant", 'squirrly-seo').'">' . esc_html__("Optimize it with SLA", 'squirrly-seo') . '</span>';
                }
