...
class SQ_Models_Bulkseo_Visibility extends SQ_Models_Abstract_Assistant
{
    ...
    public function checkNoindex($task)
    {
        ...
        if ($this->_patterns[$this->_post->post_type]['noindex']) {
            $errors[] = sprintf(esc_html__("Noindex for this post type is deactivated from %s Automation > Configuration %s.", 'squirrly-seo'), '<a href="' . SQ_Classes_Helpers_Tools::getAdminUrl('sq_automation', 'automation') . '#tab=nav-' . $this->_post->post_type . '" >', '</a>');
        }
    ...
    public function checkNofollow($task)
    {
        ...
        if ($this->_patterns[$this->_post->post_type]['nofollow']) {
            $errors[] = sprintf(esc_html__("Nofollow for this post type is deactivated from %s Automation > Configuration %s.", 'squirrly-seo'), '<a href="' . SQ_Classes_Helpers_Tools::getAdminUrl('sq_automation', 'automation') . '#tab=nav-' . $this->_post->post_type . '" >', '</a>');
        }
    ...
    public function checkNositemap($task)
    {
        ...
        if (!$this->_post->sq->do_sitemap) {
            $errors[] = sprintf(esc_html__("This post type is excluded from sitemap. See %s Automation > Configuration %s.", 'squirrly-seo'), '<a href="' . SQ_Classes_Helpers_Tools::getAdminUrl('sq_automation', 'automation') . '#tab=nav-' . $this->_post->post_type . '" >', '</a>');
        }
