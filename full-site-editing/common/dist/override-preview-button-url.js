...
async function overridePreviewButtonUrl() {
  await isEditorReady(); // Tracks when a popover is introduced into the post editor DOM
    ...
    for (const record of mutations) {
      for (const node of record.addedNodes) {
        if (isComponentsPopover(node)) {
          const previewButton = node.querySelector('a[href$="preview=true"]'); // Disables default onclick behavior for the preview button and replaces
