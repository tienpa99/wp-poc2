...
function enableStreamModeProcessing(req, onStreamRecord) {
  ...
  req.xhr.responseType = 'text'; // Find response chunks that end in a newline (possibly preceded by a carriage return), then
  ...
  let lastLine = null;
  let start = 0; // A progress event is guaranteed to be fired after the end of the response body, so we
  ...
  req.xhr.addEventListener('progress', _ref => {
    ...
    while (true) {
      ...
      lastLine = target.response.slice(start, stop); // Parse the response chunk as JSON, ignoring trailing carriage returns.
      // Note: not ignoring empty lines.
      // <https://github.com/ndjson/ndjson-spec/blob/1.0/README.md#32-parsing>

      const record = JSON.parse(lastLine); // Non-last lines should have .status == 100.
...
function request(options, fn) {
  ...
  settings.isRestAPI = options.apiNamespace === undefined; // normalize request-method name

  settings.method = settings.method.toLowerCase(); // normalize origin
  ...
  debug('API URL: %o', settings.url); // create HTTP Request instance

  const req = (superagent__WEBPACK_IMPORTED_MODULE_1___default())[method](settings.url); // querystring
