...
class HeadFooter extends Settings {
	...
	public function save_meta_box( $post_id ) {
        ...
        foreach ( $data as $key => $val ) {
			switch ($key) {
				case "head_any":
					$meta['head_any'] = isset($val) ? wp_kses(trim($val), $allowed_tags ) : '';
					break;
				case "head_desktop":
					$meta['head_desktop'] = isset($val) ? wp_kses(trim($val), $allowed_tags ) : '';
					break;
				case "head_mobile":
					$meta['head_mobile'] = isset($val) ? wp_kses(trim($val), $allowed_tags ) : '';
					break;
				case "footer_any":
					$meta['footer_any'] = isset($val) ? wp_kses(trim($val), $allowed_tags ) : '';
					break;
				case "footer_desktop":
					$meta['footer_desktop'] = isset($val) ? wp_kses(trim($val), $allowed_tags ) : '';
					break;
				case "footer_mobile":
					$meta['footer_mobile'] = isset($val) ? wp_kses(trim($val), $allowed_tags ) : '';
					break;
			}
        }
