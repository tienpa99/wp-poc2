...
class Persist
{
    ...
    public function persist()
    {
        ...
        foreach ($this->entries as $entry) {
            ...
            // Generate `VALUES` SQL
            // phpcs:disable WordPress.DB.PreparedSQL
            $rows[] = \str_ireplace(["'NULL'", '= NULL'], ['NULL', 'IS NULL'], $wpdb->prepare('%s, %s, %s, %s, %s, %s, %d, %s, %s, %d, %s', $entry->preset, $entry->blocked_url ?? 'NULL', $entry->blocked_url_host ?? 'NULL', $entry->blocked_url_hash, $entry->markup === null ? '' : $entry->markup->getId(), $entry->tag, $entry->post_id !== \false ? $post_id : 'NULL', $entry->source_url, $entry->source_url_hash, $entry->ignored ? 1 : 0, \current_time('mysql')));
            // phpcs:enable WordPress.DB.PreparedSQL
        }
