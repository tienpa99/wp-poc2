...
class Query
{
    ...
    public function getScannedPresets()
    {
        ...
        foreach ($rows as $row) {
            $preset = $row['preset'];
            // Cast result
            $result[$preset] = ['foundCount' => \intval($row['foundCount']), 'foundOnSitesCount' => \intval($row['foundOnSitesCount']), 'ignored' => \intval($row['ignored']) > 0, 'lastScanned' => \mysql2date('c', $row['lastScanned'], \false)];
        }
        return $result;
    }
    /**
