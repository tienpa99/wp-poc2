...
class Revision
{
    ...
    public function create($persist = \false, $forceNewConsent = \true)
    {
        ...
        $revision = \array_merge(['options' => $this->fromOptions(self::INDEPENDENT_OPTIONS, \false), 'groups' => Core::getInstance()->getBanner()->localizeGroups()], $this->getContextVariables());
        /**
    ...
    public function createIndependent($persist = \false)
    {
        ...
        // Create hashable revision
        $revision = ['options' => $this->fromOptions(self::INDEPENDENT_OPTIONS, \true), 'banner' => Core::getInstance()->getBanner()->getCustomize()->localizeValues(), 'blocker' => Core::getInstance()->getBlocker()->localize(), 'pageIdToPermalink' => \DevOwl\RealCookieBanner\settings\General::getInstance()->getPermalinkMap(), 'isPro' => $this->isPro(), 'isLicensed' => $isLicensed, 'isDevLicense' => $isDevLicense];
        /**
    ...
    public function getCurrent($recreate = \false)
    {
        ...
        if ($setCookiesViaManager === 'none') {
            ...
            $ids = \get_posts(Core::getInstance()->queryArguments(['post_type' => \DevOwl\RealCookieBanner\settings\Cookie::CPT_NAME, 'numberposts' => -1, 'nopaging' => \true, 'fields' => 'ids', 'meta_query' => [['key' => \DevOwl\RealCookieBanner\settings\Blocker::META_NAME_PRESET_ID, 'value' => $setCookiesViaManager === 'googleTagManager' ? PresetIdentifierMap::GTM : PresetIdentifierMap::MTM, 'compare' => '=']]], 'revisionGetManagerIds'));
         ...
         * @since 2.0.0
         */
        return \apply_filters('RCB/Revision/Current', \array_merge($create, $tagManagerResults, ['public_to_users' => $publicToUsers, 'contexts' => UserConsent::getInstance()->getPersistedContexts(), 'calculated' => $calculated, 'public_cookie_count' => \DevOwl\RealCookieBanner\settings\Cookie::getInstance()->getPublicCount(), 'all_cookie_count' => \DevOwl\RealCookieBanner\settings\Cookie::getInstance()->getAllCount(), 'all_blocker_count' => \DevOwl\RealCookieBanner\settings\Blocker::getInstance()->getAllCount(), 'cookie_counts' => \wp_count_posts(\DevOwl\RealCookieBanner\settings\Cookie::CPT_NAME), 'consents_deleted_at' => $consentsDeletedAt, 'scan_again_notice_active' => ScannerNotices::getInstance()->isNoticeAnyPluginChangeStateVisible()], $this->isPro() ? ['all_tcf_vendor_configuration_count' => TcfVendorConfiguration::getInstance()->getAllCount(), 'tcf_vendor_configuration_counts' => \wp_count_posts(TcfVendorConfiguration::CPT_NAME)] : []));
    }
    /**
