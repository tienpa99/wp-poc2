...
class CookieGroup
{
    ...
    public function ensureEssentialGroupCreated()
    {
        ...
        if ($term === null) {
            $result = \wp_insert_term(\__('Essential', Hooks::TD_FORCED), self::TAXONOMY_NAME, ['description' => $this->getDefaultDescriptions()['essential']]);
    ...
    protected function createNonEssentialDefaults()
    {
        ...
        $result = \wp_insert_term(\__('Functional', Hooks::TD_FORCED), self::TAXONOMY_NAME, ['description' => $defaultTexts['functional']]);
        \update_term_meta($result['term_id'], self::META_NAME_ORDER, 1);
        $result = \wp_insert_term(\__('Statistics', Hooks::TD_FORCED), self::TAXONOMY_NAME, ['description' => $defaultTexts['statistics']]);
        \update_term_meta($result['term_id'], self::META_NAME_ORDER, 2);
        $result = \wp_insert_term(\__('Marketing', Hooks::TD_FORCED), self::TAXONOMY_NAME, ['description' => $defaultTexts['marketing']]);
