...
class Scanner
{
    ...
    public function getServicesForNotice($max = 5)
    {
        ...
        (new BlockerPresets())->getClassList();
        (new CookiePresets())->getClassList();
        $tempTd->teardown();
        // Collect non-existing presets
        $presets = (new ScanPresets())->getAllFromCache();
        foreach ($presets as $preset) {
            if (isset($preset['created']) && $preset['created'] || \in_array($preset['identifier'], $dismissedItems, \true)) {
                continue;
            }
            $result[] = ['identifier' => $preset['identifier'], 'name' => $preset['name'], 'priority' => ($preset['scanned'] !== \false ? \strtotime($preset['scanned']['lastScanned']) : 0) + \time()];
        }
        $externalHosts = Core::getInstance()->getScanner()->getQuery()->getScannedExternalUrls();
