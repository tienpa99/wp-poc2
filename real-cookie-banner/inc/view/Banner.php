...
class Banner
{
    ...
    public function wp_head()
    {
        ...
        echo \sprintf('<style>[%s]:not(.rcb-content-blocker):not([%s]):not([%s^="children:"]){opacity:0!important;}</style>', Constants::HTML_ATTRIBUTE_BLOCKER_ID, Constants::HTML_ATTRIBUTE_UNBLOCKED_TRANSACTION_COMPLETE, Constants::HTML_ATTRIBUTE_VISUAL_PARENT);
    }
    /**
    ...
    public function wp_footer()
    {
        ...
        if ($shouldLoadAssets && !\is_customize_preview()) {
            ...
            echo \sprintf('<div id="%s" class="%s" data-bg="%s" style="%s %s position:fixed;top:0;left:0;right:0;bottom:0;z-index:999999;pointer-events:%s;display:none;filter:none;max-height:100vh;" %s></div>%s', Core::getInstance()->getPageRequestUuid4(), $antiAdBlocker ? '' : \sprintf('rcb-banner rcb-banner-%s %s', $type, empty($bgStyle) ? 'overlay-deactivated' : ''), $bgStyle, $bgStyle, $showOverlay && $this->isPro() ? \join('', \array_map(function ($prefix) use($overlayBlur) {
                return \sprintf('%sbackdrop-filter:blur(%spx);', $prefix, $overlayBlur);
            }, ['-moz-', '-o-', '-webkit-', ''])) : '', empty($bgStyle) ? 'none' : 'all', Core::getInstance()->getCompLanguage()->getSkipHTMLForTag(), \get_option(FooterDesign::SETTING_POWERED_BY_LINK) ? $this->poweredLink() : '');
        }
    }
    /**
    ...
    public function isHiddenDueLegal()
    {
        if (\get_post_type() === 'page') {
            ...
            $customize = $this->getCustomize();
            $privacyPolicy = $customize->getSetting(Legal::SETTING_PRIVACY_POLICY);
            $privacyPolicyHide = $customize->getSetting(Legal::SETTING_PRIVACY_POLICY_HIDE);
            $privacyPolicyIsExternalUrl = $customize->getSetting(Legal::SETTING_PRIVACY_POLICY_IS_EXTERNAL_URL);
            $imprint = $customize->getSetting(Legal::SETTING_IMPRINT);
            $imprintHide = $customize->getSetting(Legal::SETTING_IMPRINT_HIDE);
            $imprintIsExternalUrl = $customize->getSetting(Legal::SETTING_IMPRINT_IS_EXTERNAL_URL);
            $checkArray = [];
