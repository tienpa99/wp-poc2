...
class Scanner
{
    ...
    public function rest_api_init()
    {
        ...
        \register_rest_route($namespace, '/scanner/result/presets', ['methods' => 'GET', 'callback' => [$this, 'routeResultPresets'], 'permission_callback' => [$this, 'permission_callback']]);
        ...
        \register_rest_route($namespace, '/scanner/result/externals/preset/(?P<preset>[a-zA-Z0-9_-]+)', ['methods' => 'GET', 'callback' => [$this, 'routeResultAllExternalUrlsByPreset'], 'permission_callback' => [$this, 'permission_callback']]);
        ...
        \register_rest_route($namespace, '/scanner/result/notice-dismissed', ['methods' => 'DELETE', 'callback' => [$this, 'dismissScannerNotice'], 'permission_callback' => [$this, 'permission_callback'], 'args' => ['notice_type' => ['notice_type' => 'enum', 'enum' => ScannerNotices::NOTICE_TYPES, 'required' => \true]]]);
    ...
    {
        $result = Core::getInstance()->getScanner()->getQuery()->getAllScannedExternalUrlsBy('preset', $request->get_param('preset'));
        return \count($result) > 0 ? new WP_REST_Response(['items' => $result]) : new WP_Error('rest_not_found', 'Preset not found');
    }
    /**
     * See API docs.
     *
    ...
    {
        return ['presets' => $this->routeResultPresets()->get_data(), 'externalUrls' => $this->routeResultExternalUrls()->get_data()];
    }
    /**
    ...
    {
        return new WP_REST_Response(['success' => ScannerNotices::getInstance()->dismiss($request->get_param('notice_type'))]);
    }
    /**
     * New instance.
     */
