...
class Assets
{
    ...
    public function enqueue_scripts_and_styles($type, $hook_suffix = null)
    {
        ...
        $this->anonymous_localize_script($handle, 'realCookieBanner', $this->localizeScript($type), [Cookie::META_NAME_CODE_OPT_IN, Cookie::META_NAME_CODE_OPT_OUT, Cookie::META_NAME_CODE_ON_PAGE_LOAD], !\in_array($type, [Constants::ASSETS_TYPE_FRONTEND, Constants::ASSETS_TYPE_LOGIN], \true) && !\is_customize_preview());
    }
    /**
    ...
    public function enqueueAdminPage($scriptDeps)
    {
        ...
        // react-router-dom
        \array_unshift($scriptDeps, $this->enqueueLibraryScript('react-router-dom', [[$useNonMinifiedSources, 'react-router-dom/umd/react-router-dom.js'], 'react-router-dom/umd/react-router-dom.min.js'], [Constants::ASSETS_HANDLE_REACT_DOM]));
        // @antv/g2
    ...
    public function enqueueBanner($scriptDeps)
    {
        ...
        if (\is_customize_preview() || $hasAnimations) {
            $this->enqueueLibraryStyle('animate-css', [[$useNonMinifiedSources, 'animate.css/animate.css'], 'animate.css/animate.min.css']);
        }
        ...
        \wp_add_inline_script($handle, '((a,b)=>{a[b]||(a[b]={},["consentSync"].forEach(c=>a[b][c]=()=>({cookie:null,consentGiven:!1,cookieOptIn:!0})),["consent","consentAll","unblock"].forEach(c=>a[b][c]=(...d)=>new Promise(e=>a.addEventListener(b,()=>{a[b][c](...d).then(e)},{once:!0}))))})(window,"consentApi");', 'before');
    ...
    public function overrideLocalizeScript($context)
    {
        ...
        if ($context === Constants::ASSETS_TYPE_ADMIN) {
            ...
            $result = ['showLicenseFormImmediate' => $showLicenseFormImmediate, 'showNoticeAnonymousScriptNotWritable' => DeliverAnonymousAsset::getContentDir() === \false, 'assetsUrl' => $core->getAdInitiator()->getAssetsUrl(), 'customizeBannerUrl' => $bannerCustomize->getUrl(), 'adminUrl' => \admin_url(), 'colorScheme' => $colorScheme, 'cachePlugins' => CacheInvalidator::getInstance()->getLabels(), 'modalHints' => ModalHints::getInstance()->getAlreadySeen(), 'isDemoEnv' => \DevOwl\RealCookieBanner\DemoEnvironment::getInstance()->isDemoEnv(), 'isConfigProNoticeVisible' => $core->getConfigPage()->isProNoticeVisible(), 'activePlugins' => RealProductManagerUtils::getActivePluginsMap(), 'iso3166OneAlpha2' => Iso3166OneAlpha2::getSortedCodes(), 'predefinedCountryBypassLists' => CountryBypass::PREDEFINED_COUNTRY_LISTS, 'defaultCookieGroupTexts' => CookieGroup::getInstance()->getDefaultDescriptions(\true), 'useEncodedStringForScriptInputs' => \version_compare($wp_version, '5.4.0', '>='), 'resetUrl' => \add_query_arg(['_wpnonce' => \wp_create_nonce('rcb-reset-all'), 'rcb-reset-all' => 1], $core->getConfigPage()->getUrl())];
        ...
        return \apply_filters('RCB/Localize', \array_merge($result, $this->localizeFreemiumScript(), ['hasDynamicPreDecisions' => \has_filter('RCB/Consent/DynamicPreDecision'), 'isLicensed' => $isLicensed, 'isDevLicense' => $isDevLicense, 'multilingualSkipHTMLForTag' => $core->getCompLanguage()->getSkipHTMLForTag(), 'isCurrentlyInTranslationEditorPreview' => $core->getCompLanguage()->isCurrentlyInEditorPreview(), 'defaultLanguage' => $core->getCompLanguage()->getDefaultLanguage(), 'currentLanguage' => $core->getCompLanguage()->getCurrentLanguage(), 'context' => Revision::getInstance()->getContextVariablesString(), 'userConsentCookieName' => \DevOwl\RealCookieBanner\MyConsent::getInstance()->getCookieName(), 'revisionHash' => Revision::getInstance()->getCurrentHash(), 'isTcf' => TCF::getInstance()->isActive(), 'isPreventPreDecision' => $banner->isPreventPreDecision(), 'isAcceptAllForBots' => $consentSettings->isAcceptAllForBots(), 'isRespectDoNotTrack' => $consentSettings->isRespectDoNotTrack(), 'isEPrivacyUSA' => $consentSettings->isEPrivacyUSAEnabled(), 'isAgeNotice' => $consentSettings->isAgeNoticeEnabled(), 'isListServicesNotice' => $consentSettings->isListServicesNoticeEnabled(), 'setCookiesViaManager' => $generalSettings->getSetCookiesViaManager(), 'essentialGroup' => CookieGroup::getInstance()->getEssentialGroup()->slug, 'groups' => $banner->localizeGroups(), 'blocker' => $core->getBlocker()->localize(), 'setVisualParentIfClassOfParent' => Blocker::SET_VISUAL_PARENT_IF_CLASS_OF_PARENT, 'dependantVisibilityContainers' => Blocker::DEPENDANT_VISIBILITY_CONTAINERS, 'bannerDesignVersion' => Banner::DESIGN_VERSION, 'bannerI18n' => \array_merge($core->getCompLanguage()->translateArray([
            ...
            'durationUnit' => ['s' => \__('second(s)', RCB_TD), 'm' => \__('minute(s)', RCB_TD), 'h' => \__('hour(s)', RCB_TD), 'd' => \__('day(s)', RCB_TD), 'mo' => \__('month(s)', RCB_TD), 'y' => \__('year(s)', RCB_TD)],
        ...
        ], [], null, ['legal-text'])), 'pageRequestUuid4' => $core->getPageRequestUuid4(), 'pageByIdUrl' => \add_query_arg('page_id', '', \home_url()), 'pageIdToPermalink' => $generalSettings->getPermalinkMap(), 'pageId' => \get_post_type() === 'page' ? \get_the_ID() : \false, 'pluginUrl' => $core->getPluginData('PluginURI')]), $context);
    }
    /**
