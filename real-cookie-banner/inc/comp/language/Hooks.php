...
class Hooks
{
    ...
    {
        return TemporaryTextDomain::fromPluginReceiver(self::TD_FORCED, RCB_TD, Core::getInstance(), $this->compInstance(), Localization::class);
    }
    /**
    ...
    public function copy_blocker_cookies_meta($meta_value, $from, $to, $locale)
    {
        if ($this->compInstance()->isActive() && !empty($meta_value)) {
            ...
            foreach (\explode(',', $meta_value) as $currentId) {
                $translationId = $this->compInstance()->getCurrentPostId($currentId, Blocker::CPT_NAME, $locale);
