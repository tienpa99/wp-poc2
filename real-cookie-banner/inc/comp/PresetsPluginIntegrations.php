...
class PresetsPluginIntegrations
{
    ...
    public function middleware_cookies_recommended(&$preset)
    {
        ...
        $preset['recommended'] = \apply_filters('RCB/Presets/Cookies/Recommended', $this->presets_cookies_recommended($preset['recommended'] ?? \false, $preset['id']), $preset);
        return $preset;
    }
    /**
     * Automatically set the `recommended` attribute to `true` for some special cases.
    ...
    {
        // Documented in RecommendedHooksMiddleware
        $preset['recommended'] = \apply_filters('RCB/Presets/Blocker/Recommended', $this->presets_blocker_recommended($preset['recommended'] ?? \false, $preset['id']), $preset);
        return $preset;
    }
    /**
    ...
    {
        switch ($identifier) {
            case PresetIdentifierMap::WORDPRESS_USER_LOGIN:
                $recommended = \get_option(self::OPTION_NAME_USERS_CAN_REGISTER) > 0;
                break;
            case PresetIdentifierMap::CLOUDFLARE:
                $recommended = isset($_SERVER['HTTP_CF_CONNECTING_IP']) && !empty($_SERVER['HTTP_CF_CONNECTING_IP']);
                break;
            case PresetIdentifierMap::EZOIC_ESSENTIAL:
            case PresetIdentifierMap::EZOIC_MARKETING:
            case PresetIdentifierMap::EZOIC_PREFERENCES:
            case PresetIdentifierMap::EZOIC_STATISTIC:
                $recommended = isset($header['x-middleton']);
                break;
            case PresetIdentifierMap::JETPACK_SITE_STATS:
                if (\class_exists(Jetpack::class) && \method_exists(Jetpack::class, 'is_module_active')) {
                    $recommended = Jetpack::is_module_active('stats');
                }
                break;
            case PresetIdentifierMap::WOOCOMMERCE_GEOLOCATION:
             ...
             */
            case PresetIdentifierMap::MONSTERINSIGHTS_4:
                if (\function_exists('monsterinsights_get_v4_id') && !empty(\monsterinsights_get_v4_id())) {
                    $recommended = \true;
                }
                break;
            case PresetIdentifierMap::EXACT_METRICS_4:
    ...
    public function presets_plugin_false_positives($isActive, $plugin, $identifier, $type)
    {
        switch ($plugin) {
            ...
            case self::SLUG_WOOCOMMERCE_GOOGLE_ANALYTICS_FREE:
                ...
                return isset($option['ga_id'], $option['ga_standard_tracking_enabled']) && !empty($option['ga_id']) && $option['ga_standard_tracking_enabled'] !== 'no';
                break;
    ...
    {
        return self::$me === null ? self::$me = new \DevOwl\RealCookieBanner\comp\PresetsPluginIntegrations() : self::$me;
    }
}
