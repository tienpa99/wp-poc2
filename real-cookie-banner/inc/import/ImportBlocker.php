    ...
    protected function doImportBlocker($blockers)
    {
        ...
        foreach ($blockers as $index => $blocker) {
            ...
            if (\is_array($metas)) {
                ...
                if (isset($metas[Blocker::META_NAME_VISUAL_MEDIA_THUMBNAIL])) {
            ...
            if (!$this->isPro() && isset($metas[Blocker::META_NAME_PRESET_ID]) && !empty($metas[Blocker::META_NAME_PRESET_ID])) {
                $presetWithAttributes = (new BlockerPresets())->getWithAttributes($metas[Blocker::META_NAME_PRESET_ID]);
                if ($presetWithAttributes === \false || $presetWithAttributes['tier'] !== AdoptTierFromClassNamespaceMiddleware::TIER_FREE) {
