...
trait ImportSettings
{
    ...
    protected function handleSepcialSetting($optionName, $value, $key)
    {
        $onlyPro = \false;
        switch ($optionName) {
            case General::SETTING_IMPRINT_ID:
            case General::SETTING_PRIVACY_POLICY_ID:
            case General::SETTING_HIDE_PAGE_IDS:
                if ($value > 0 || !empty($value)) {
                    $this->addMessageOptionRelatesPageId(($optionName === General::SETTING_PRIVACY_POLICY_ID ? \__('Privacy policy page', RCB_TD) : $optionName === General::SETTING_IMPRINT_ID) ? \__('Imprint page', RCB_TD) : \__('Hide on additional pages', RCB_TD), $key);
                    break;
                }
                return \true;
