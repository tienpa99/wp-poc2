...
class ScriptInlineMatch extends AbstractMatch
{
    ...
    public function isScriptOnlyVariableAssignment($variableNames = [], $compute = \true)
    {
        ...
        foreach ($variableNames as $variable) {
            ...
            } elseif (Utils::startsWith($trimmedScript, \sprintf('var %s', $variable)) || Utils::startsWith($trimmedScript, \sprintf('window.%s', $variable)) || Utils::startsWith($trimmedScript, \sprintf('document.%s', $variable))) {
                return \true;
            }
        }
