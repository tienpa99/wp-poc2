...
class Persist
{
    ...
    const DECLARATION_TYPES = [self::DECLARATION_TYPE_PURPOSES, self::DECLARATION_TYPE_SPECIAL_PURPOSES, self::DECLARATION_TYPE_FEATURES, self::DECLARATION_TYPE_SPECIAL_FEATURES];
    /**
    ...
    const VENDOR_OVERWRITE_FIELDS = ['name', 'purposes', 'legIntPurposes', 'flexiblePurposes', 'specialPurposes', 'features', 'specialFeatures', 'policyUrl', 'usesCookies', 'cookieMaxAgeSeconds', 'cookieRefresh', 'usesNonCookieAccess', 'deviceStorageDisclosureUrl', 'deviceStorageDisclosureViolation', 'deviceStorageDisclosure', 'additionalInformation'];
    ...
    public function normalizeDeclarations($language, &$vendorList, &$translation)
    {
        ...
        foreach (self::DECLARATION_TYPES as $declarationType) {
            ...
            foreach ($purposes as $idx => $purpose) {
                ...
                $row = \array_merge(['gvlSpecificationVersion' => $gvlSpecificationVersion, 'tcfPolicyVersion' => $tcfPolicyVersion, 'id' => $purpose['id'], 'language' => $language, 'name' => $purpose['name'], 'description' => $purpose['description'], 'descriptionLegal' => \sanitize_textarea_field($purpose['descriptionLegal'])], $purposeTranslation);
                // Generate SQL
                // phpcs:disable WordPress.DB.PreparedSQL
                $rows[] = $wpdb->prepare('%d, %d, %d, %s, %s, %s, %s', $row['gvlSpecificationVersion'], $row['tcfPolicyVersion'], $row['id'], $row['language'], $row['name'], $row['description'], $row['descriptionLegal']);
                // phpcs:enable WordPress.DB.PreparedSQL
            }
    ...
    public function normalizeStacks($language, &$vendorList, &$translation)
    {
        ...
        foreach ($stacks as $idx => $stack) {
            ...
            // Generate row with translated content
            $row = \array_merge(['gvlSpecificationVersion' => $gvlSpecificationVersion, 'tcfPolicyVersion' => $tcfPolicyVersion, 'id' => $stack['id'], 'language' => $language, 'name' => $stack['name'], 'description' => $stack['description'], 'descriptionLegal' => \sanitize_textarea_field($stack['descriptionLegal'] ?? ''), 'purposes' => $stack['purposes'], 'specialFeatures' => $stack['specialFeatures']], $stackTranslation);
            // Generate SQL
            // phpcs:disable WordPress.DB.PreparedSQL
            $rows[] = $wpdb->prepare('%d, %d, %d, %s, %s, %s, %s, %s, %s', $row['gvlSpecificationVersion'], $row['tcfPolicyVersion'], $row['id'], $row['language'], $row['name'], $row['description'], $row['descriptionLegal'], \join(',', $row['purposes']), \join(',', $row['specialFeatures']));
            // phpcs:enable WordPress.DB.PreparedSQL
        }
    ...
    protected function persistDeclaration($declarationType, &$rows)
    {
        ...
        $overwriteSql = [];
        $fieldsSql = \join(',', ['gvlSpecificationVersion', 'tcfPolicyVersion', 'id', 'language', 'name', 'description', 'descriptionLegal']);
    ...
    protected function persistVendors(&$rows)
    {
        ...
        // Allow to update fields if already exists
        $overwriteSql = [];
        $fieldsSql = \join(',', ['vendorListVersion', 'id', 'name', 'purposes', 'legIntPurposes', 'flexiblePurposes', 'specialPurposes', 'features', 'specialFeatures', 'policyUrl', 'usesCookies', 'cookieMaxAgeSeconds', 'cookieRefresh', 'usesNonCookieAccess', 'deviceStorageDisclosureUrl', 'deviceStorageDisclosureViolation', 'deviceStorageDisclosure', 'additionalInformation']);
    ...
    protected function persistStacks(&$rows)
    {
        ...
        // Allow to update fields if already exists
        $overwriteSql = [];
        $fieldsSql = \join(',', ['gvlSpecificationVersion', 'tcfPolicyVersion', 'id', 'language', 'name', 'description', 'descriptionLegal', 'purposes', 'specialFeatures']);
