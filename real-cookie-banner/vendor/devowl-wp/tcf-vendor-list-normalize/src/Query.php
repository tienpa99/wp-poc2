...
class Query
{
    ...
    public function allDeclarations($args = [])
    {
        ...
        if ($gvlSpecificationVersion === null || $tcfPolicyVersion === null) {
            list($gvlSpecificationVersion, $tcfPolicyVersion) = $this->getLatestUsedPurposeVersions(Persist::DECLARATION_TYPE_PURPOSES);
        }
    ...
    public function stacks($args = [])
    {
        ...
        // Query latest versions
        if ($gvlSpecificationVersion === null || $tcfPolicyVersion === null) {
            list($gvlSpecificationVersion, $tcfPolicyVersion) = $this->getLatestUsedPurposeVersions(Persist::TABLE_NAME_STACKS);
        }
        // Query purposes for current language
        // phpcs:disable WordPress.DB.PreparedSQL
        $rows = $wpdb->get_results($wpdb->prepare("SELECT *\n                FROM {$table_name}\n                WHERE language = %s\n                    AND gvlSpecificationVersion = %d\n                    AND tcfPolicyVersion = %d", $language, $gvlSpecificationVersion, $tcfPolicyVersion), ARRAY_A);
    ...
    public function vendors($args = [])
    {
        ...
        // Query purposes for current language
        // phpcs:disable WordPress.DB.PreparedSQL
        $rows = $wpdb->get_results($wpdb->prepare("SELECT *\n                FROM {$table_name}\n                WHERE vendorListVersion = %d\n                {$inSql}\n                ORDER BY name ASC", $vendorListVersion), ARRAY_A);
        // phpcs:enable WordPress.DB.PreparedSQL
    ...
    public function vendor($vendorId, $args = [])
    {
        ...
        // Query purposes for current language
        // phpcs:disable WordPress.DB.PreparedSQL
        $row = $wpdb->get_row($wpdb->prepare("SELECT *\n                FROM {$table_name}\n                WHERE vendorListVersion = %d\n                AND id = %d\n                ORDER BY name ASC", $vendorListVersion, $vendorId), ARRAY_A);
        // phpcs:enable WordPress.DB.PreparedSQL
        if (!\is_array($row)) {
            // Nothing found.
            return \false;
    ...
    protected function castReadDeclaration($rows)
    {
        $result = [];
        foreach ($rows as $row) {
            $newRow = ['id' => \intval($row['id']), 'name' => $row['name'], 'description' => $row['description'], 'descriptionLegal' => $row['descriptionLegal']];
    ...
    protected function castReadVendors($rows)
    {
        $result = [];
        foreach ($rows as &$row) {
            $newRow = ['id' => \intval($row['id']), 'name' => $row['name'], 'policyUrl' => $row['policyUrl'], 'usesCookies' => \boolval($row['usesCookies']), 'cookieMaxAgeSeconds' => \intval($row['cookieMaxAgeSeconds']), 'cookieRefresh' => \boolval($row['cookieRefresh']), 'usesNonCookieAccess' => \boolval($row['usesNonCookieAccess'])];
            ...
            // Explode purposes and features to array
            foreach (['purposes', 'legIntPurposes', 'flexiblePurposes', 'specialPurposes', 'features', 'specialFeatures'] as $purposeType) {
                $newRow[$purposeType] = \array_filter(\array_map('intval', \explode(',', $row[$purposeType])));
            }
            $result[$row['id']] = $newRow;
        }
    ...
    public function getLatestUsedPurposeVersions($type)
    {
        ...
        $table_name = $this->getNormalizer()->getTableName($type);
        // phpcs:disable WordPress.DB.PreparedSQL
        return \array_map('intval', \array_values($wpdb->get_row("SELECT MAX(gvlSpecificationVersion), MAX(tcfPolicyVersion) FROM {$table_name}", ARRAY_A)));
        // phpcs:enable WordPress.DB.PreparedSQL
    }
    /**
