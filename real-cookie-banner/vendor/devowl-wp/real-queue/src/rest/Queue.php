...
class Queue
{
    ...
    public function rest_api_init()
    {
        ...
        \register_rest_route($namespace, '/job/(?P<id>[0-9]+)', ['methods' => 'GET', 'callback' => [$this, 'routeJobGet'], 'permission_callback' => [$this, 'permission_callback'], 'args' => ['lock' => ['type' => 'boolean']]]);
        \register_rest_route($namespace, '/job/(?P<id>[0-9]+)', ['methods' => 'POST', 'callback' => [$this, 'routeJobExecute'], 'permission_callback' => [$this, 'permission_callback'], 'args' => ['try' => ['type' => 'string', 'sanitize_callback' => function ($value) {
            return empty($value) ? [] : \array_map('intval', \explode(',', $value));
        }]]]);
        \register_rest_route($namespace, '/job/(?P<id>[0-9]+)/result', ['methods' => 'POST', 'callback' => [$this, 'routeJobResult'], 'permission_callback' => [$this, 'permission_callback'], 'args' => ['process' => ['type' => 'number', 'required' => \true, 'sanitize_callback' => function ($value) {
            return \intval($value);
        }], 'errorCode' => ['type' => 'string'], 'errorMessage' => ['type' => 'string'], 'errorData' => ['type' => 'string']]]);
        \register_rest_route($namespace, '/jobs', ['methods' => 'GET', 'callback' => [$this, 'routeJobsGet'], 'permission_callback' => [$this, 'permission_callback'], 'args' => ['after' => ['type' => 'number'], 'ids' => ['type' => 'string', 'sanitize_callback' => function ($value) {
            return empty($value) ? [] : \array_map('intval', \explode(',', $value));
        }]]]);
        \register_rest_route($namespace, '/jobs', ['methods' => 'DELETE', 'callback' => [$this, 'routeJobsDelete'], 'permission_callback' => [$this, 'permission_callback'], 'args' => ['type' => ['type' => 'string']]]);
        \register_rest_route($namespace, '/jobs/retry', ['methods' => 'POST', 'callback' => [$this, 'routeJobsRetry'], 'permission_callback' => [$this, 'permission_callback'], 'args' => ['type' => ['type' => 'string']]]);
        \register_rest_route($namespace, '/jobs/skip', ['methods' => 'POST', 'callback' => [$this, 'routeJobsSkip'], 'permission_callback' => [$this, 'permission_callback'], 'args' => ['type' => ['type' => 'string']]]);
        \register_rest_route($namespace, '/status', ['methods' => 'GET', 'callback' => [$this, 'routeStatus'], 'permission_callback' => [$this, 'permission_callback'], 'args' => ['additionalData' => ['type' => 'string']]]);
    }
    /**
    ...
    public function routeStatus($request)
    {
        ...
        return new WP_REST_Response(['currentJobs' => $query->readCurrentJobs(\true), 'errors' => $executor->buildErrorMessages($remaining), 'remaining' => $remaining, 'additionalData' => $additionalData ? $this->fetchAdditionalData(\explode(',', $request->get_param('additionalData'))) : (object) []]);
    }
    /**
