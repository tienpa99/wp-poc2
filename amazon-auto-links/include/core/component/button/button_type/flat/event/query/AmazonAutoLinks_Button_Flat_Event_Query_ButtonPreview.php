...
/**
 * Outputs the `flat` button preview.
 *
 * @since 5.2.0
 */
class AmazonAutoLinks_Button_Flat_Event_Query_ButtonPreview extends AmazonAutoLinks_Button_Event_Query_ButtonPreview_Base {

    /**
     * @var   array     Supported button types
     * @since 5.2.0
     */
    public $aButtonTypes = array( 'flat' );
    ...
    protected function _getBodyTagInnerHTML( $isButtonID, $sButtonLabel ) {
        return "<div id='preview-button' data-button-id='" . esc_attr( $isButtonID ) . "'>"
                . $this->getButton( $isButtonID, $sButtonLabel, true, false, 'flat' )
            . "</div>";
    }
    ...
    protected function _getExtraResourcesAddedToHead( $sHeadTagInnerHTML ) {
        ...
        $_sStylesheetURL2   = $this->getSRCFromPath( AmazonAutoLinks_Button_Flat_Loader::$sDirPath . "/asset/css/button-flat-preview-framed" . $_sMin . ".css" );
        ...
        $sHeadTagInnerHTML .= "<link rel='stylesheet' id='button-flat-preview-framed-style' href='" . esc_url( $_sStylesheetURL2 ) . "' media='all'>";
