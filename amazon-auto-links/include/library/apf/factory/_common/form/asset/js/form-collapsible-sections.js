...
( function( $ ) {
    ...
    $( document ).on( 'amazon-auto-links_saved_widget', function( event, oWidget ){
        $( oWidget ).initializeAmazonAutoLinks_AdminPageFrameworkCollapsibleSections();
    });
    ...
    $.fn.initializeAmazonAutoLinks_AdminPageFrameworkCollapsibleSections = function() {
        ...
        $( this ).find( '.amazon-auto-links-collapsible-sections-title[data-is_collapsed=\"0\"]' )
            .next( '.amazon-auto-links-collapsible-sections-content' )
            .slideDown( 'fast' );
        $( this ).find( '.amazon-auto-links-collapsible-section-title[data-is_collapsed=\"0\"]' )
        ...
        $( this ).find( '.amazon-auto-links-collapsible-section-title[data-is_collapsed=\"1\"]' )
            .closest( '.amazon-auto-links-section-table' )
            .find( 'tbody' )
            .hide();

        // Bind the click event to the title element.
        $( this ).find( '.amazon-auto-links-collapsible-sections-title, .amazon-auto-links-collapsible-section-title' )
            .enableAmazonAutoLinks_AdminPageFrameworkCollapsibleButton();

        // Insert the toggle all button.
        $( this ).find( '.amazon-auto-links-collapsible-title[data-toggle_all_button!=\"0\"]' ).each( function(){$

            var _oThis        = $( this ); // to access from inside the below each() method.
            var _bForSections = $( this ).hasClass( 'amazon-auto-links-collapsible-sections-title' );   // or for the 'section' container.
            ...
            $.each( _aPositions, function( iIndex, _sPosition ) {
                ...
                _oButton.on( 'click', function(){
                    ...
                    var _oButtons = _bForSections
                        ? $( this ).closest( '.amazon-auto-links-sectionset' ).siblings().addBack().find( '> .amazon-auto-links-collapsible-toggle-all-button-container' )
                        : $( this ).siblings( '.amazon-auto-links-collapsible-toggle-all-button-container' ).addBack();
        ...
        function _getButtonContainer() {
            var _sDashIconSort = $.fn.compareVersionNumbers( AmazonAutoLinks_AdminPageFrameworkScriptFormMain.wpVersion, '3.8' ) >= 0
                ? 'dashicons dashicons-sort'
                : '';
            var _sButtonText   = _sDashIconSort ? '' : AmazonAutoLinks_AdminPageFrameworkScriptFormMain.messages.toggleAll;
            var _oButtonInner = $( '<span class="amazon-auto-links-collapsible-toggle-all-button button"></span>' );
            _oButtonInner.addClass( _sDashIconSort );
            _oButtonInner.attr( 'title', AmazonAutoLinks_AdminPageFrameworkScriptFormMain.messages.toggleAllCollapsibleSections );
            _oButtonInner.text( _sButtonText );
            var _oButton = $( '<div class="amazon-auto-links-collapsible-toggle-all-button-container"></div>' );
    ...
    $.fn.enableAmazonAutoLinks_AdminPageFrameworkCollapsibleButton = function() {
        ...
        function _isFieldElement( nodeTarget ) {
            ...
            if ( $( nodeTarget ).hasClass( 'amazon-auto-links-collapsible-button' ) ) {
                return false;
            }
        ...
        $( this ).on( 'click', function( event, sContext ){
            ...
            // Expand or collapse this panel
            var _oThis = $( this );
            var _sContainerType = $( this ).hasClass( 'amazon-auto-links-collapsible-sections-title' )
                ? 'sections'
                : 'section';
            var _oTargetContent = 'sections' === _sContainerType
                ? $( this ).next( '.amazon-auto-links-collapsible-content' ).first()
            ...
            if ( 'expand' === _sAction && _oThis.data( 'collapse_others_on_expand' ) ) {
                _oThis.parent().parent().children().children( '* > .amazon-auto-links-collapsible-content' ).not( _oTargetContent ).slideUp( 'fast', function() {
                    $( this ).prev( '.amazon-auto-links-collapsible-title' ).addClass( 'collapsed' );
                });
            }
