...
( function( $ ) {
     ...
     * @since    3.7.0
     */
    $( document ).on( 'amazon-auto-links_saved_widget', function( event, oWidget ){
        jQuery( '.amazon-auto-links-form-loading' ).remove();
    });
...
(function ( $ ) {
     ...
     */
    $( document ).on( 'amazon-auto-links_added_repeatable_field', function( oEvent, sFieldType, sID, iCallType, iSectionIndex, iFieldIndex ){
     ...
     */
    $( document ).on( 'amazon-auto-links_repeated_field', function( oEvent, iCallType, oModelContainer ){
     ...
     * @since       3.8.0
     */
    $( document ).on( 'amazon-auto-links_stopped_sorting_sections', function( oEvent ){
     ...
     */
    /* $( document ).on( 'amazon-auto-links_removed_field', function( oEvent, sFieldType, sID, iCallType, iSectionIndex, iFieldIndex ){
        var _oThisNode = jQuery( oEvent.target );
...
(function ( $ ) {
    ...
    $( document ).ready( function() {
        ...
        $( '.amazon-auto-links-fields.repeatable' ).each( function() {
            ...
            /* Otherwise, insert the button element at the beginning of the field tag */
                 ...
                 */
                if ( ! _childFields.find( '.amazon-auto-links-repeatable-field-buttons' ).length ) {
             ...
             */
            $( this ).find( '> .amazon-auto-links-field.with-child-fields' ).prepend( _buttonsNestedFields );
                /**
                 * Add buttons to the fields tag.
                 */
                // $( this ).find( '.amazon-auto-links-field.with-nested-fields' ).prepend( _buttonsNestedFields );

                /**
                 * Support for inline mixed fields.
                 * @todo not sure why this is commented out. Remove this if it's okay.
                 */
                // $( this ).find( '.amazon-auto-links-field.with-mixed-fields' ).prepend( _buttonsNestedFields );




            $( this ).updateAmazonAutoLinks_AdminPageFrameworkRepeatableFields( _aSettings ); // Update the fields
     ...
     * @var AmazonAutoLinks_AdminPageFrameworkScriptFormMain
     */
    var translation = AmazonAutoLinks_AdminPageFrameworkScriptFormMain;
    ...
    $.fn.addAmazonAutoLinks_AdminPageFrameworkRepeatableField = function( sFieldContainerID ) {
        ...
        if ( 'undefined' === typeof sFieldContainerID ) {
            sFieldContainerID = $( this ).closest( '.amazon-auto-links-field' ).attr( 'id' );
        }
...
( function( $ ) {
    ...
    $( document ).ready( function() {
        ...
        $( '.amazon-auto-links-sections.repeatable-section' ).each( function( index ){

            var _buttonModel    = $( this ).children( '.repeatable-section-buttons-model' ).first();
            var _repeatOptions  = _buttonModel.data();
            _buttonModel = _buttonModel.find( '.amazon-auto-links-repeatable-section-buttons' ).first().detach();

            // Add buttons
            $( this ).find( '.amazon-auto-links-section-caption' ).each( function(){

                $( this ).show();
                var _oButtons = $( _buttonModel[0].outerHTML );
                if ( $( this ).children( '.amazon-auto-links-collapsible-section-title' ).children( 'fieldset' ).length > 0 ) {
                    _oButtons.addClass( 'section_title_field_sibling' );
                }
                var _oCollapsibleSectionTitle = $( this ).find( '.amazon-auto-links-collapsible-section-title' );
            ...
            // Update the fields
            $( this ).updateAmazonAutoLinks_AdminPageFrameworkRepeatableSections( _repeatOptions );
    ...
    $.fn.addAmazonAutoLinks_AdminPageFrameworkRepeatableSection = function( sSectionContainerID ) {
        ...
        if ( 'undefined' === typeof sSectionContainerID ) {
            sSectionContainerID = $( this ).closest( '.amazon-auto-links-section' ).attr( 'id' );
        }
        ...
        var _incrementAttributes = function( oElement, iSectionsCount, oSectionsContainer ) {
            ...
            $( oElement ).find( 'tr.amazon-auto-links-fieldrow, .amazon-auto-links-fieldset, .amazon-auto-links-fields, .amazon-auto-links-field, table.form-table, input,textarea,select,option' )
                .incrementAttribute(
                    'id',
                    iSectionsCount,
                    _sSectionIDModel
...
( function( $ ) {
  ...
  $( document ).ready( function() {
    ...
    $( '.amazon-auto-links-section-tabs-contents' ).createTabs();
  });
...
(function($) {
    ...
    // Initialize
    $( document ).ready( function() {
        $( '.amazon-auto-links-fields.sortable' ).each( function() {
            $( this ).enableAmazonAutoLinks_AdminPageFrameworkSortableFields();
        } );
    });
...
(function($) {
    ...
    // Initialization
    $( document ).ready( function() {
        $( '.amazon-auto-links-sections.sortable-section' ).each( function(){
            $( this ).enableAmazonAutoLinks_AdminPageFrameworkSortableSections();
        } );
    });
...
(function($){
  ...
  $.widget( 'amazon-auto-links.pointerTooltip', $.wp.pointer, {
   /**
 ...
 * To use the method,
 * ```
 * $( '.my-tooltip' )[ 'amazon-auto-links-form-tooltip' ]( 'Hi, this is a tooltip content!' );
 * ```
 * Or have a text message enclosed in an element with a class `amazon-auto-links-form-tooltip-content`.
 * ```
 *    <span class="my-tooltip dashicons dashicons-editor-help">
 *      <span class="amazon-auto-links-form-tooltip-content">
 *        Some text
 *      </span>
 *    </span>
 * ```
 * Then, call it like
 * ```
 * $( '.my-tooltip' )[ 'amazon-auto-links-form-tooltip' ]();
 * ```
 * Or
 * ```
 ...
 * So not to call the `[ 'amazon-auto-links-form-tooltip' ]()` method, just create an element with the selector
...
(function($){
  ...
  // Initialize
  $( document ).ready( function() {
    $( '.amazon-auto-links-form-tooltip' ).each( function() {
      $( this )[ 'amazon-auto-links-form-tooltip' ]();
    } );
  } );

  $.fn[ 'amazon-auto-links-form-tooltip' ] = function( options ) {
  ...
  function initialize( target, options ) {
    ...
    options = $.extend( {}, {
      pointerClass: 'amazon-auto-links-form-tooltip-balloon',
  ...
  function handleTooltip( self, options ) {
    ...
    var _content   = 'undefined' !== typeof options.content
      ...
      : $( self ).find( '.amazon-auto-links-form-tooltip-content' ).clone();
  ...
  function handleAutoClose( self, content ) {
      ...
      $( self ).add( '.amazon-auto-links-form-tooltip-balloon' ).on( 'mouseleave', function( event ){
      ...
      $( self ).add( '.amazon-auto-links-form-tooltip-balloon' ).on( 'mouseenter', function(){
