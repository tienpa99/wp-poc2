...
(function($){
  ...
  var apfContact = AmazonAutoLinks_AdminPageFrameworkContactFieldType;
  ...
  function sendEmail( event ) {
    ...
    // Check required
    var _requiredInputs = $( this ).closest( '.amazon-auto-links-sections' )
    ...
    var _oSpinner = $('<img src="' + apfMain.spinnerURL + '" alt="' + apfMain.messages.loading + '" />' )
      .addClass( 'amazon-auto-links-ajax-spinner' );
    $( this ).closest( '.amazon-auto-links-field' ).find( '.result-placeholder' ).prepend( _oSpinner );
    var _resultIcon = $( this ).closest( '.amazon-auto-links-field' ).find( '.result-placeholder .dashicons' );
    ...
    $.ajax( {
        ...
        success: function ( response ) {
          ...
          if ( response.result ) {
            _resultIcon.addClass( 'dashicons-yes-alt success amazon-auto-links-form-tooltip' );
            _resultIcon.prepend( "<span class='amazon-auto-links-form-tooltip-content'>" + response.message + "</span>" );
          } else {
            _resultIcon.addClass( 'dashicons-warning error amazon-auto-links-form-tooltip' );
            _resultIcon.prepend( "<span class='amazon-auto-links-form-tooltip-content'>" + response.message + "</span>" );
          }
        },
        error: function( response ) {
          ...
          _resultIcon.addClass( 'dashicons-warning error amazon-auto-links-form-tooltip' );
          _resultIcon.prepend( "<span class='amazon-auto-links-form-tooltip-content'>" + response.status + ' ' + response.statusText + "</span>" );
        },
        complete: function() {
          _oSpinner.remove();
          debugLog( 'Request done.' );
          _resultIcon[ 'amazon-auto-links-form-tooltip' ]({
              autoClose: false,
              shown: true,
              oneOff: true,
              noArrow: true,
