...
( function ( $ ) {
    ...
    apfRevealerOnChange = function() {
        ...
        if ( $( this ).is( ':checkbox' ) ) {
            _oElementToReveal       = _bGlobal
                ? $( _sTargetSelector )
                : $( this ).closest( '.amazon-auto-links-section' ).find( _sTargetSelector );
            if ( $( this ).is( ':checked' ) ) {
        ...
        var _oAllSubjects = _bGlobal
            ? $( _sSelectors )
            : $( this ).closest( '.amazon-auto-links-section' ).find( _sSelectors );
        _oAllSubjects.hide();
  ...
  $( document ).ready( function(){
    ...
    $.each( apfRevealerFieldType.fields, function( inputID, field ) {
      ...
      _oInputs.setAmazonAutoLinks_AdminPageFrameworkRevealer();
